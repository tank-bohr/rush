...
RSpec.describe Rush::AST::List do
  # A real executor (errexit off) runs the real errexit context for async entries.
  ...
  it 'runs each entry in order and returns the last status' do
    ...
    expect(executor.errexit).not_to have_received(:tested)
  end
  ...
  it 'launches an async entry in a tested context, returning its launch status' do
    ...
    expect(executor.errexit).to have_received(:tested).once

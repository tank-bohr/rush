...
not Sorbet's current seven RBI inputs or the executable. Its current receiver-based ledger is 11,863 /
11,891 typed calls (99.76%). That percentage is not comparable to Sorbet's send metric; even their input
...
- `sorbet/coverage_budgets.json` owns pass/fail policy: after the fourth `rush-435.7` root slice, at
  most 1,052 untyped sends and an exact rational minimum ratio of 12,151 / 13,203. Both checks apply, so codebase growth cannot hide a
...
3. **Missing ordinary declarations.** Slice 19u typed every method in `TestOperators`,
   ...
   operator tables. Many remaining typed-true classes assign ivars without `T.let`, so Sorbet still
   propagates untyped through otherwise ordinary calls. `rush-435.7` should continue with shared
   root declarations and remeasure before changing dispatch architecture.
...
4. **Untyped value readers and bounded variants.** Slice 19t gave the three `ulimit` values exact
   ...
   object; Slice 19w did the same for `ShellProcessIds`, and hand-wrote the three Forwardable reads
   that feed shell-parameter resolution. Non-AST `Data.define` values in `getopts`, `cd`, `HereDoc`
   and `ExitTrap` still expose untyped readers. Other explicit opens are domain variants: lexer token kind/value,
   `AST::WordSegment` payloads, redirect targets, getopts' `String | :keep | nil`, logical IO streams,
   and generic block returns. These should become named unions or value contracts where consumers
   discriminate them; broad casts would only move the counter.
...
The explicit source ledger currently contains 115 `T.untyped` declaration lines across production
and project RBIs (38 production files and five RBI files), plus three `T.unsafe` and seven `T.cast`
...
escape only for measured native/Racc/variant residue. After the fourth `rush-435.7` root slice, at
the normal denominator:

- `ceil(13,203 × 0.95) = 12,543` typed sends;
- this needs 392 additional typed sends;
- no more than 660 sends may remain untyped;
- that removes 37.3% of the current 1,052-send gap.
...
conservative scope envelope of 12,168 / 13,324 typed sends (91.32%, gap 1,156). At that denominator
95% is 12,658 typed sends: +490, leaving at most 666 untyped, a 42.4% gap reduction. The current
location probe finds 741 concrete send-shaped diagnostics (740 outside generated parser), with 362
concentrated in 22 files having at least ten each. These diagnostics still do not map one-for-one to
...
precise boundaries leave more than 666 sends untyped at the expanded scope, `rush-435.9` must publish

...
not Sorbet's current seven RBI inputs or the executable. Its current receiver-based ledger is 11,997 /
12,025 typed calls (99.77%). That percentage is not comparable to Sorbet's send metric; even their input
...
- `sorbet/coverage_budgets.json` owns pass/fail policy: after the ninth `rush-435.7` root slice, at
  most 912 untyped sends and an exact rational minimum ratio of 12,240 / 13,152. Both checks apply, so codebase growth cannot hide a
...
escape only for measured native/Racc/variant residue. After the ninth `rush-435.7` root slice, at
the normal denominator:

- `ceil(13,152 × 0.95) = 12,495` typed sends;
- this needs 255 additional typed sends;
- no more than 657 sends may remain untyped;
- that removes 28.0% of the current 912-send gap.
...
conservative scope envelope of 12,257 / 13,273 typed sends (92.35%, gap 1,016). At that denominator
95% is 12,610 typed sends: +353, leaving at most 663 untyped, a 34.7% gap reduction. The current
location probe finds 662 concrete send-shaped diagnostics (661 outside generated parser), with 293
